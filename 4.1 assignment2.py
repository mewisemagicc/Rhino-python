        ...
        for j in range(JMAX):
            for k in range(KMAX):
                if i > 0 and j > 0 and k > 0:
                    ...
                    if quadNum == 1:
                        crvFront = rs.AddCurve((pts[0],pts[4],pts[5],pts[1],pts[0]),3)
                        profile = rs.AddLine(ptDict[(i-1,j,k-1)], pts[0])
                    if quadNum == 2:
                        crvFront = rs.AddCurve((pts[1],pts[5],pts[6],pts[2],pts[1]),3)
                        profile = rs.AddLine(ptDict[(i-1,j,k-1)], pts[1])
                    if quadNum == 3:
                        crvFront = rs.AddCurve((pts[2],pts[6],pts[7],pts[3],pts[2]),3)
                        profile = rs.AddLine(ptDict[(i-1,j,k-1)], pts[2])
                    if quadNum == 4:
                        crvFront = rs.AddCurve((pts[4],pts[8],pts[9],pts[5],pts[4]),3)
                        profile = rs.AddLine(ptDict[(i-1,j,k-1)], pts[4])
                    if quadNum == 5:
                        crvFront = rs.AddCurve((pts[5],pts[9],pts[10],pts[6],pts[5]),3)
                        profile = rs.AddLine(ptDict[(i-1,j,k-1)], pts[5])
                    if quadNum == 6:
                        crvFront = rs.AddCurve((pts[6],pts[10],pts[11],pts[7],pts[6]),3)
                        profile = rs.AddLine(ptDict[(i-1,j,k-1)], pts[6])
                    if quadNum == 7:
                        crvFront = rs.AddCurve((pts[8],pts[12],pts[13],pts[9],pts[8]),3)
                        profile = rs.AddLine(ptDict[(i-1,j,k-1)], pts[8])
                    if quadNum == 8:
                        crvFront = rs.AddCurve((pts[9],pts[13],pts[14],pts[10],pts[9]),3)
                        profile = rs.AddLine(ptDict[(i-1,j,k-1)], pts[9])
                    if quadNum == 9:
                        crvFront = rs.AddCurve((pts[10],pts[14],pts[15],pts[11],pts[10]),3)
